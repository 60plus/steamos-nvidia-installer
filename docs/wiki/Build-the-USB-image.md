...
parent with `--workdir /path/to/new-build-directory`. The directory itself must not
already exist. The output image and `.sha256` file are written next to the input.
Existing output is refused, not overwritten.

The recovery root is mounted read-only; compilation changes go into a separate
OverlayFS layer. Package signatures remain enabled. On success, compilation
residue is removed after unmounting; artifact directories, driver cache and
`build.log` remain. On failure, the script stops, detaches its mounts and retains
the work directory for diagnosis. Do not flash incomplete output or remove a
work directory until mounts and loop devices have been checked.

For another trusted updater configuration, use `--source /path/to/source.json`.
The default public package's supported OS list is unchanged by this builder.
Run `--help` for driver selection and other options.

The complete wrapper has built an image from the original 3.8.14 recovery image
in an Arch Linux QEMU guest on Windows. All four artifacts were compiled during
that run. Read-only output checks passed for the driver, addon integrity,
32-bit/64-bit NVIDIA libraries and updater configuration, with xpadneo absent.
The output checksum was generated by the wrapper. The tester subsequently
confirmed installation, the OS update, controller operation and Remote Play in
both directions. HEVC Video was enabled manually in Advanced Client Options;
see [Remote Play client settings](Troubleshooting.md#remote-play-client-codec-setting).
These results cover the tested setup, not every client or hardware configuration.

A small real Btrfs/OverlayFS fixture also checks failure cleanup, unchanged input
...
For a base-only or manual build, use the included public configuration with
...
## CUDA trimming and Remote Play

`--trim-cuda` filters CUDA, OpenCL, NVVM and OptiX paths from the main builder's
payload list. It is not a switch to disable Remote Play or NVENC. However, the
project's 64-bit NVENC helper dynamically loads `libcuda.so.1`, so removing that
runtime library can prevent hardware encoding. A normal loader dependency check
does not cover all libraries loaded dynamically at runtime.

Inspection of the complete test image confirmed that matching `libcuda`,
`libnvidia-encode` and `libnvcuvid` libraries are present despite trimming being
requested. Do not interpret the flag as proof that the final image contains no
CUDA runtime, or assume every streaming implementation works without it.
Use `--keep-cuda` with the complete builder, or omit `--trim-cuda` with the base
builder, when retaining compute libraries. Validate both Remote Play directions
after installation and OS updates.
